...
private:
  ...
  bool usingSoftSerial = false;
  SoftwareSerial *varSoftSerial;
	HardwareSerial *varSerial;

  inline size_t sendData(unsigned char *buf, unsigned int len) {
    if (usingSoftSerial == true) {
      varSoftSerial->write(buf, len);
    } else {
      varSerial->write(buf, len);
    }
  }
  ...
  inline virtual int availableData () {
    if (usingSoftSerial == true) {
      return varSoftSerial->available();
    } else {
      return varSerial->available();
    }
  }

  inline virtual int readData () {
    if (usingSoftSerial == true) {
      return varSoftSerial->read();
    } else {
      return varSerial->read();
    }
  }

  inline virtual int peekData () {
    if (usingSoftSerial == true) {
      return varSoftSerial->peek();
    } else {
      return varSerial->peek();
    }
  }

  inline void beginCom (unsigned long baud) {
    if (usingSoftSerial == true) {
      varSoftSerial->begin(baud);
    } else {
      varSerial->begin(baud);
    }
  }
...
public:
  void begin(long baud, unsigned char directionPin, SoftwareSerial *srl);
